# -*- coding: utf-8 -*-
"""
Created on Wed Feb 21 22:08:37 2024
...
from LV_element_orientation import cal_element_orientation
#%%
...
#%%
class LVFEModel:
    def __init__(self, node_x, element, node_X, boundary, pressure_surface,
        ...
        if element.shape[1] == 4:
            ...
            self.state['n_integration_points_solid']=4     # 8(slow) or 4(fast)
            self.state['n_integration_points_surface']=1   # 3(slow) or 1(fast)
        ...
        free_node=np.setdiff1d(free_node, boundary.view(-1).numpy())
    ...
    def initialize_for_inflation(self):
        ...
        state["disp"]=0 #zero displacement of boundary
    ...
    def set_element_orientation(self, element_orientation):
        if element_orientation is not None:
            element_orientation=element_orientation.to(self.dtype).to(self.device)
        self.state['element_orientation']=element_orientation
        
    def set_material(self, material): 
        if material is not None:
            material=material.to(self.dtype).to(self.device)
        self.state['material']=material

    def set_node_x(self, node_x):
        if node_x is not None:
            node_x=node_x.to(self.dtype).to(self.device)
    ...
    def set_boundary_displacement(self, disp):
        #prescribed displacement of boundary
        if disp is not None:
            disp=disp.to(self.dtype).to(self.device)
        self.state["disp"]=disp

    def set_F0(self, F0):
        #pre/residual deformation
        if F0 is not None:
            F0=F0.to(self.dtype).to(self.device)            
        self.state['F0']=F0
    ...
    def get_u_field(self):
        ...
        if state['node_X'] is not None:
            ...
            u_field=torch.zeros_like(state['node_x'], dtype=self.dtype, device=self.device)
        u_field[state['boundary']]=state["disp"]
    ...
    def cal_energy_and_force_for_inflation(self, pressure, return_stiffness=None):
        ...
        #force_ext2 is exernal force on boundary
        #it is passive: it will adjust itself to match force_int on boundary
        ...
        # TPE2 contains the passive energy on boundary and should not be used for linesearch
        # two special cases:
