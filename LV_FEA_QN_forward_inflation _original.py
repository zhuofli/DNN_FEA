# -*- coding: utf-8 -*-
"""
Created on Wed Feb 21 22:12:44 2024
...
import sys
sys.path.append("C:/Users/zhuofli/OneDrive - Texas Tech University/Data_Zhuofan/NNFEA/code_original/code/torch_fea")
...
#%%
all_mat=torch.load('C:/Users/zhuofli/OneDrive - Texas Tech University/Data_Zhuofan/NNFEA/data/data/125mat.pt')['mat_str']
matMean=torch.load('C:/Users/zhuofli/OneDrive - Texas Tech University/Data_Zhuofan/NNFEA/data/data/125mat.pt')['mean_mat_str']
px_pressure=4
mat_model='HO'
#mat_str="1e3, 0, 1, 0, 0, 1e5"; mat_name='1e3'
#mat_str="1e2, 0, 1, 0, 0, 1e5"; mat_name='1e2'
#mat_str=matMean; mat_name='matMean'
#mat_str='generate_mat_distribution(3, arg.mesh_p0)'; mat_name='distribution3'
#mesh_p0_str='D:/MLFEA/minliang_lv/data/ori/p54_c3d4_ori'
mat_str='generate_mat_distribution(4,arg.mesh_p0)'; mat_name='distribution4_4fixed_newcontrols'
mesh_p0_str='C:/Users/zhuofli/OneDrive - Texas Tech University/Data_Zhuofan/NNFEA/data/data/Zhuofan result/x2_c3d10_ori'
...
parser.add_argument('--pressure_noise_std', default=0, type=float)
...
parser.add_argument('--save_as_vtk', default='True', type=str)#True: save *.vtk file in addition to *.pt file
...
arg.save_as_vtk = True if arg.save_as_vtk == 'True' else False
...
#%%
from LVFEModel import LVFEModel, cal_potential_energy
#------------------
...
elif arg.mat_model == "HO": # 
    from torch_fea.material.Mat_HO_4p_Jv import cal_1pk_stress
else:
...
if 'HO' in arg.mat_model:
                if 'distribution' in arg.mat:
                    ...
                    Mat=torch.tensor([Mat], dtype=dtype, device=device)
#%%
LV_model=LVFEModel(None, Element, Node_X, Boundary, Element_surface_pressure,
                   Mat, ElementOrientation, cal_1pk_stress, dtype, device, mode='inflation')
pressure=arg.pressure
'''
if arg.pressure_noise_std > 0:
    print("non-uniform pressure with mean="+str(arg.pressure)+" and std="+str(arg.pressure_noise_std))
    rng=np.random.RandomState(0)#reproducible random noise
    noise=rng.randn(len(Element_surface_pressure))
    pressure=pressure+arg.pressure_noise_std*torch.tensor(noise, dtype=dtype, device=device)
...
if len(arg.mesh_px_init) > 0:
    ...
    u_field_init=u_field_init[LV_model.state['free_node']]
    print("initialize u_field with u_field_init")
    LV_model.state['u_field'].data=u_field_init.to(dtype).to(device)
    ...
    if 0 <= arg.t_start_init <= 1:
        ...
        for τ in t_start_list:
            out=LV_model.cal_energy_and_force(pressure*τ)
            TPE1=out['TPE1']; TPE2=out['TPE2']; SE=out['SE']
            force_int=out['force_int']; force_ext=out['force_ext']
            F=out['F']; u_field=out['u_field']
...
def save(is_final_result=False):
    ...
    u_field=LV_model.get_u_field()
    Node_x=Node_X+u_field
    S=LV_model.cal_stress('cauchy', create_graph=False, return_W=False)    
    F=LV_model.cal_F_tensor()
    I=torch.eye(3).expand(1,1,3,3).to(F.dtype).to(F.device)
    E=0.5*(torch.matmul(F.permute(0,1,3,2), F)-I)
    E_element=E.mean(dim=1)
    S_element=S.mean(dim=1)    
    S_node=cal_attribute_on_node(Node_x.shape[0], Element, S_element)
    E_node=cal_attribute_on_node(Node_x.shape[0], Element, E_element)
    
    VM_element=cal_von_mises_stress(S_element)
    MaxAbsPS_element=cal_max_abs_principal_stress(S_element)
    MaxAbsPE_element=cal_max_abs_principal_strain(E_element)    
    
    VM_node=cal_von_mises_stress(S_node)
    MaxAbsPS_node=cal_max_abs_principal_stress(S_node)    
    MaxAbsPE_node=cal_max_abs_principal_strain(E_node)
   
    Mat_element=Mat.reshape(-1,6).expand(len(Element),6)
    ...
    Mesh_x.element_data['Mat']=Mat_element.detach().cpu()
    Mesh_x.element_data['S']=S_element.reshape(-1,9).detach().cpu()
    Mesh_x.element_data['MaxAbsPS']=MaxAbsPS_element.reshape(-1,1).detach().cpu()
    Mesh_x.element_data['MaxAbsPE']=MaxAbsPE_element.reshape(-1,1).detach().cpu()
    Mesh_x.element_data['VM']=VM_element.reshape(-1,1).detach().cpu()
    Mesh_x.node_data['Mat']=Mat_node.detach().cpu()
    Mesh_x.node_data['S']=S_node.reshape(-1,9).detach().cpu()
    Mesh_x.node_data['VM']=VM_node.reshape(-1,1).detach().cpu()
    Mesh_x.node_data['MaxAbsPS']=MaxAbsPS_node.reshape(-1,1).detach().cpu()
    Mesh_x.node_data['MaxAbsPE']=MaxAbsPE_node.reshape(-1,1).detach().cpu()
...
optimizer = LBFGS([LV_model.state['u_field']], lr=1, line_search_fn="backtracking",
...
def reform_stiffness(pressure_t):
    ta=time.time()
    out=LV_model.cal_energy_and_force(pressure_t, return_stiffness="sparse")
    H=out['H']
    H=H.astype("float64")
    #print("type(H)", type(H))
...
#%%
t_good=t_start
Ugood=LV_model.state['u_field'].clone().detach()
#save mesh at t=0
...
while iter1 <= max_iter1:
    ...
    while iter2 <= max_iter2:
        ...
        def closure(output=closure_opt["output"], loss1_fn=closure_opt["loss1_fn"]):
            out=LV_model.cal_energy_and_force(pressure_t)
            TPE1=out['TPE1']; TPE2=out['TPE2']; SE=out['SE']
            force_int=out['force_int']; force_ext=out['force_ext']
            F=out['F']; u_field=out['u_field']
        ...
        #-------------------------------------------------
        U1=LV_model.state['u_field'].clone().detach()
        opt_cond=optimizer.step(closure)
        U2=LV_model.state['u_field'].clone().detach()
        ...
        if error_flag==1:
            ...
            if error_iter2 is None:
                ...
                LV_model.state['u_field'].data=U1
                continue
            else:
        ...
        if opt_cond is True:
            ...
            pressure_t_print=pressure_t
            if torch.is_tensor(pressure_t):
                pressure_t_print=pressure_t.mean().item()
            print("  t", auto_stepper.t, "iter2", iter2, "pressure_t", pressure_t_print, "time", time_list[-1])
    ...
    if opt_cond is True:
        ...
        LV_model.state['u_field'].data=Ugood.clone().detach()
        retry_counter+=1
